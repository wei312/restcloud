...
import OpenApi from '../form/OpenApi';
...
class ListMyTestApis extends React.Component {
  constructor(props) {
    super(props);
    this.state={
  ...
  loadData=(pagination=this.state.pagination,filters={},sorter={},searchFilters={})=>{
    let url=LIST_URL+"?dataType=TEST";
  ...
  cancelFollowApi=()=>{
    ...
    if(ids===''){AjaxUtils.showError("请选择一个要删除的API!");return;}
        ...
        <div style={{ background: '#fff',padding:25,borderRadius:'4px'}} >
           <BackTop />
             <Row style={{marginBottom:5}} gutter={0} >
                 <Col span={12} >
                   <Button  type="primary" onClick={AjaxUtils.showConfirm.bind(this,"删除确认","确认删除测试记录?",this.cancelFollowApi)} icon="delete"  disabled={!hasSelected}  loading={loading} >删除记录</Button>{' '}
