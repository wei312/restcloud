...
import NewCategoryType from '../form/NewCategoryType';
import ListCategoryNode from './ListCategoryNode';

const Search = Input.Search;
const confirm = Modal.confirm;
const LIST_URL=URI.CORE_CATEGORYTYPE.list; //分页显示
const DELETE_URL=URI.CORE_CATEGORYTYPE.delete;//删除
const ButtonGroup = Button.Group;

class ListCategoryType extends React.Component {
  constructor(props) {
    super(props);
    this.url=LIST_URL;
    this.state={
  ...
  render(){
    ...
    const expandedRow=(record)=>{
      return (<Card><ListCategoryNode categoryId={record.categoryId} appId={record.appId} rootName={record.categoryName}/></Card>);
    }

    return (
      <Card title="系统分类管理" style={{minHeight:600}}>
	          <Modal key={Math.random()} title='分类管理属性' maskClosable={false}
                visible={this.state.visible}
                width='800px'
                footer=''
                onOk={this.handleCancel}
                onCancel={this.handleCancel} >
                <NewCategoryType id={currentId} close={this.closeModal} />
            </Modal>
