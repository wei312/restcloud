...
class ListReportApiAccessLog extends React.Component {
  ...
  loadData=()=>{
    ...
    let url=LIST_URL+"?appId="+this.state.appId+"&startDate="+this.startDate+"&endDate="+this.endDate;
  ...
  render(){
    ...
    const columns=[{
      ...
      render:(text,record)=>{
        let stateTags;
        if(record.state==='2'){
          stateTags=<Tag >调试</Tag>;
        }else if(record.state==='3'){
          stateTags=<Tag  >停用</Tag>;
        }else if(record.state==='4'){
          stateTags=<Tag >模拟</Tag>;
        }else{
          stateTags=<Tag  >启用</Tag>;
        }
        return stateTags;
      }
    },{
