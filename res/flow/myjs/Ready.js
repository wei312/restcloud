...
jsPlumb.ready(function() {
	...
	$('#Container').click(function() {
		...
		//画出矩形
		if (h < 0 && w >= 0) {
            $("#multipleSelectedRectangle").css({ "height": (-h) + "px", "width": w + "px", "position": "absolute", "left": px + scrollX - 0 + "px", "top": pyy + scrollY - 0 + "px", "opacity": "0.2", "border": "1px dashed #000" });
        }
        else if (h >= 0 && w < 0) {
            $("#multipleSelectedRectangle").css({ "height": h + "px", "width": (-w) + "px", "position": "absolute", "left": pxx + scrollX - 0 + "px", "top": py + scrollY - 0 + "px", "opacity": "0.2", "border": "1px dashed #000" });
        }
        else if (h < 0 && w < 0) {
            $("#multipleSelectedRectangle").css({ "height": (-h) + "px", "width": (-w) + "px", "position": "absolute", "left": pxx + scrollX - 0 + "px", "top": pyy + scrollY - 0 + "px", "opacity": "0.2", "border": "1px dashed #000" });
        }
        else {
            $("#multipleSelectedRectangle").css({ "height": h + "px", "width": w + "px", "position": "absolute", "left": px + scrollX - 0 + "px", "top": py + scrollY - 0 + "px", "opacity": "0.2", "border": "1px dashed #000" });
        }
        if (w < 0) {
            w = 0 - w;
        }
	  ...
	  if(event.button==2){
			...
			if(newNodeType!==''){
				...
				var left = mousePos.x-50;
				createNewNode(newNodeType,{ 'top': top, 'left': left });
				newNodeType='';//lch清除新增节点状态
				mouseTool(); //lch点击右键取消链接变化移动状态
			}
		}
	 ...
	 */
	// var processModelObj;
	// var processModel =window.loadProcessModel();
	...
	// }
	// console.log(processModelObj);
	// console.log(obj);
	// initFlowCharts(processModelObj);
