...
function createNewNode(nodeType, p) {
	...
	//添加到画板中
	$("#Container").append('<div id="' + newNode.newId + '" class="' + newNode.cla + '" ondblclick="editProperty(\'' + newNode.newId + '\')"></div>');
	addTextToNode($("#" + newNode.newId),newNode.text);


	//设置节点位置
	$("#" + newNode.newId).offset( { top: newNode.locTop, left: newNode.locLeft } );
	...
	switch (createType) {
		...
		case 2:
		  //通过拖拽创建新节点
			$("#" + newNode.newId).css('height', parseInt($("#" + newNode.newId).css('height')) + 10 + 'px');
			$("#" + newNode.newId).css('width', parseInt($("#" + newNode.newId).css('width')) + 10 + 'px');
...
function createLane(newNode) {
	...
	//设置节点位置
	var t = newNode.nodeType == 'broadwiseLane' ? newNode.locTop : 81;
	var l = newNode.nodeType == 'broadwiseLane' ? 201 : newNode.locLeft;
...
function chooseNodeObjFromType(type, p) {
	...
	switch(type) {
		...
		case 'taskNode':
			...
			nodeObj.cla = "taskNode moveLight";
			nodeObj.nodeType = 'taskNode';
			nodeObj.text = "任务节点";
			nodeObj.bgColor = '#ccab26';
			break;
		case 'apiNode':
				nodeObj.newId = getNextNodeId('T');
				nodeObj.cla = "apiNode moveLight";
		...
		case 'computeNode':
					nodeObj.newId = getNextNodeId('E');
					nodeObj.cla = "computeNode moveLight";
					nodeObj.nodeType = 'computeNode';
					nodeObj.text = "";
					nodeObj.bgColor = '#ccab26';
					break;
		case 'eventNode':
		...
		case 'poolNode':
					nodeObj.newId = getNextNodeId('E');
					nodeObj.cla = "poolNode moveLight";
	  ...
	  case 'dataNode':
					nodeObj.newId = getNextNodeId('T');
					nodeObj.cla = "dataNode moveLight";
					nodeObj.nodeType = 'dataNode';
					nodeObj.text = "数据模型";
					nodeObj.bgColor = '#ccab26';
					break;
		case 'readDataNode':
					nodeObj.newId = getNextNodeId('T');
					nodeObj.cla = "readDataNode moveLight";
					nodeObj.nodeType = 'readDataNode';
					nodeObj.text = "数据模型";
					nodeObj.bgColor = '#ccab26';
					break;
    case 'mockNode':
					nodeObj.newId = getNextNodeId('T');
					nodeObj.cla = "mockNode moveLight";
					nodeObj.nodeType = 'mockNode';
					nodeObj.text = "模拟数据";
					nodeObj.bgColor = '#ccab26';
					break;
		case 'scriptNode':
			nodeObj.newId = getNextNodeId('T');
			nodeObj.cla = "scriptNode moveLight";
			nodeObj.nodeType = 'scriptNode';
			nodeObj.text = "脚本节点";
			nodeObj.bgColor = '#ccab26';
			break;
		case 'gateWay':
		...
		case 'joinNode':
				nodeObj.newId = getNextNodeId('E');
				nodeObj.cla = "joinNode moveLight";
				nodeObj.nodeType = 'joinNode';
				nodeObj.text = "集合点";
				nodeObj.bgColor = '#7faab3';
				nodeObj.icon = '';
				break;
		case 'subProcess':
		...
		case 'fileNode':
			nodeObj.newId = getNextNodeId('T');
			nodeObj.cla = "fileNode moveLight";
			nodeObj.nodeType = 'fileNode';
			nodeObj.text = "文件";
			nodeObj.bgColor = '#fceb00';
			nodeObj.icon = '';
			break;
...
function initFlowCharts(o) {
	...
	for(var i = 0; i < linkArr.length; i++) {
		...
		if (linkArr[i].label != '') {
			...
			// connLineObj.endpoints[0].setAnchor(['Top']);
			// connLineObj.endpoints[1].setAnchor(['Top']);
		}
...
function saveObj(obj) {
	...
	$.ajax({
	    url: "/bpm/rest/ws/p/S018/handleFlowJSON/created",
	    //同步请求
	    async: false,
	    type: 'POST',
	    data: post_data,
...
function getCurrentFlowDoc() {
	...
	$.each($('#Container').children('.moveLight'), function(index) {
		var tempObj = {};
		// console.log(graph.node($(this).attr('id')));
...
function showGrid() {
	if ($("#canvasId").css('background-image') == 'none') {
		...
		$("#showGridId").children(':first-child').attr('class', 'fa fa-eye fa-2x showItemTxt');
		$("#showGridId").children(':last-child').text('隐藏网格');
	} else {
		$("#canvasId").css('background', 'none');
		// $("#canvasId").css('background-color', '#F8F8F8');
		$("#showGridId").children(':first-child').attr('class', 'fa fa-eye-slash fa-2x showItemTxt');
...
function registerNodeEvent(tempId,nodeObj) {
	...
	$(selector).mouseover(function() {
		...
		if (!graph.node($(this).attr('id')).isSelected) {
			...
			// $(this).css('border', "4px solid #40AFFE");
		}
	}).mouseout(function() {
		// 当节点选中标识为false，也就是未被选中时
		if (!graph.node($(this).attr('id')).isSelected) {
			$(this).css('box-shadow', NODE_NO_SELECTED_BOX_SHADOW);
			// $(this).css('border', "2px solid #40AFFE");
		}
...
function saveAllDefaultNode() {
	...
	var s = '/bpm/rule?wf_num=R_S002_B011&Action=SaveAllDefaultNode&Processid=' + processId + '&Nodeid=';
